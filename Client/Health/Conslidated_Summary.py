...
def summaries():
    q3 = """select sum(Normalized_Earned_Premium) as EARNED_PREMIUM, 
           sum(Normalized_POLICIES_EXPOSED) as POLICIES_EXPOSED, sum(Normalized_LIVES_EXPOSED) as LIVES_EXPOSED,             
            norm_policy.Zone,Mem_Gender,Renewal_Count,
            norm_policy.Sum_Insured, Mem_Age, Product_name,  
            Channel_type, Revised_Individual_Floater 
            from norm_policy
            group by norm_policy.Zone , Mem_Gender , norm_policy.Sum_Insured, Mem_Age, 
            Product_name,Channel_type, Revised_Individual_Floater,Renewal_Count            
     """
...
def merge_policies():
    ...
    for file_name in files:
        financial_year = file_name[39:43]
...
member_claim = policy.merge(member, on=["Policy_number"], how="left")
member_count = db.sql(
...
norm_policy = member_claim.merge(nep, on="Policy_number")
