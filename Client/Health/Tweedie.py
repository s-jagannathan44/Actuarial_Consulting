...
ordinal_list = ["Mem_Age_New", "Mem_Gender_New", "Revised_Product_Name_New", "Renewal_Count_New", "Financial_Year",
...
def execute_model(tweedie_model, test_set, dataframe):
    ...
    make_multi(dataframe, "Mem_Age_New Mem_Gender_New Revised_Product_Name_New Renewal_Count_New  "
                          "Financial_Year Zone_New Sum_Insured_New".split())


# -------------------- CODE STARTS HERE ---------------------------------------
df = pd.read_csv("CSV\\FrequencyModelFile.csv", usecols="Mem_Age_New Mem_Gender_New LIVES_EXPOSED "
                                                        "Financial_Year PAID_AMT Renewal_Count_New "
                                                        "Revised_Product_Name_New Zone_New Sum_Insured_New"
                 .split())
...
exposure = df_train["LIVES_EXPOSED"],
X_train = df_train.drop(["PAID_AMT", "Loss_Cost", "LIVES_EXPOSED"], axis=1)
X_test = df_test.drop(["PAID_AMT", "Loss_Cost", "LIVES_EXPOSED"], axis=1)
X_val = df_val.drop(["PAID_AMT", "Loss_Cost", "LIVES_EXPOSED"], axis=1)

rgr = XGBRegressor(objective='reg:tweedie', tweedie_variance_power=1.5, n_estimators=500,
...
(pd.Series(rgr.feature_importances_,
           index=["Mem_Age_New", "Mem_Gender_New", "Revised_Product_Name_New", "Financial_Year", "Renewal_Count_New",
                  "Zone_New", "Sum_Insured_New"])
 .nlargest(7)
 .plot(kind='barh'))
