...
def data_verification():
    ...
    print("training Claim average", train['Claim'].mean())
    print("test Claim average", test['Claim'].mean())
    # Step 8 Check the sum of Exposure and sum  of Claim in the train and test sets.
    print(train.Claim.describe(percentiles=[0.25, 0.5, 0.75, 0.95, 1]))
    print('\n-------------------------\n')
    print(test.Claim.describe(percentiles=[0.25, 0.5, 0.75, 0.95, 1]))


# Step 1 read File
freq = pd.read_csv('Output\\FMTPL.csv')
freq = freq[freq["Claim"] > 0]
freq = freq.drop(freq[freq["Claim"] > 5000].index)
# freq["Claim"] = freq["Claim"].clip(upper=5000)
freq = freq.reset_index(drop=True)
freq.describe(percentiles=[0.25, 0.5, 0.75, 0.95, 1]).to_csv("Output\\description.csv")
freq.corr().to_csv("Output\\cross_correlation.csv")
...
df_freq = freq.iloc[freq.drop(['IDpol', 'Exposure', 'ClaimNb', 'Claim'], axis=1).drop_duplicates().index]
...
df_freq_ml = Ut.motor_third_party_severity_transform(df_freq)
