...
# for index in range(len(master)):
#     if index < len(master):
#         UY = master["UY Original"].iloc[index]
#         Flag = master["LT_ANNUAL Flag"].iloc[index]
#         Body_Type = master["Body Type"].iloc[index]
#         Age = master["V AGE BAND"].iloc[index]
#         CC = master["CC_Original"].iloc[index]
#         Make = master["Vehicle Make"].iloc[index]
#         State = master["Registration States"].iloc[index]
#         Zone = master["Zone"].iloc[index]
#         cost = unroll.loc[
#             (unroll['UY Original'] == UY) & (unroll['LT_ANNUAL Flag'] == Flag) & (unroll['V AGE BAND'] == Age) &
#             (unroll['Body Type'] == Body_Type) & (unroll['CC_Original'] == CC) &
#             (unroll['Registration States'] == State) & (unroll['Vehicle Make'] == Make) &
#             (unroll['Zone'] == Zone), 'Death Predicted']
#         cost = np.sum(cost)
#         if cost is not np.nan:
#             master["Death Predicted"].iloc[index] = np.sum(cost)
#         else:
#             master["Death Predicted"].iloc[index] = 0
#
#         cost = unroll.loc[
#             (unroll['UY Original'] == UY) & (unroll['LT_ANNUAL Flag'] == Flag) & (unroll['V AGE BAND'] == Age) &
#             (unroll['Body Type'] == Body_Type) & (unroll['CC_Original'] == CC) &
#             (unroll['Registration States'] == State) & (unroll['Vehicle Make'] == Make) &
#             (unroll['Zone'] == Zone), 'Death Actual']
#         cost = np.sum(cost)
#         if cost is not np.nan:
#             master["Death Actual"].iloc[index] = np.sum(cost)
#         else:
#             master["Death Actual"].iloc[index] = 0
#         print(index)
#         print(cost)
# master.to_csv("Output\\merged.csv")
